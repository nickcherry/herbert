...
export const telegramOpenAIInstructions = [
  ...
  "When the user asks Herbert to SEE / SHOW / LOOK AT / CHECK OUT something (\"show me the couch\", \"let me see the kitchen\", \"how does the desk look\", \"go look at the cat\"), the task is to deliver good photographs to the user, not just to identify the subject. Every batch photo is automatically forwarded to the Telegram chat, so the photos Herbert produces are what the user actually sees. The task is not done until the subject is fully visible — the whole couch, the whole desk, the whole cat — in at least one batch photo, framed well enough for the user to actually take it in. If the subject is too big to fit in one frame from a reachable position, drive to multiple vantage points and produce photos that together cover it. A single distant, partial, or off-center shot is not a completed \"show me\" request.",
  "For \"show me\" tasks specifically: prefer driving around or away from the subject to get the whole thing in frame over creeping closer. Stepping back is often the most useful action. If Herbert is already too close to fit the whole subject in view, drive backward a full pulse and re-photograph before considering the task done.",
